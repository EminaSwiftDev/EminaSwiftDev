...
struct CustomPlan48View: View {
    @Environment(\.dismiss) var dismiss
    @EnvironmentObject var zenFlowManager: ZenFlowManager
    @State private var fastingHours: Double = 24
    @State private var eatingHours: Double = 24
    @State private var showError = false
    
    private let hoursRange = Array(1...47)
            ...
            VStack(spacing: 30) {
                ...
                VStack(spacing: 10) {
                    ...
                    Text("\(Int(fastingHours + eatingHours))")
                        ...
                        .foregroundColor(fastingHours + eatingHours == 48 ? .green : .red)
                    
                    Text(fastingHours + eatingHours == 48 ? "Perfect! 48 hours" : "Must equal 48 hours")
                        .font(.subheadline)
                        .foregroundColor(fastingHours + eatingHours == 48 ? .green : .red)
                }
                .padding()
                .background(
                    RoundedRectangle(cornerRadius: 15)
                        .fill(fastingHours + eatingHours == 48 ? Color.green.opacity(0.1) : Color.red.opacity(0.1))
                )
                ...
                .padding(.horizontal)
                .disabled(fastingHours + eatingHours != 48)
                .opacity(fastingHours + eatingHours == 48 ? 1.0 : 0.5)
                
                Spacer()
            }
            .padding()
        }
        .navigationTitle("Custom 48h Plan")
